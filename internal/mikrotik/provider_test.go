...
func TestMikrotikProvider_ApplyChanges(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			changes: &plan.Changes{
				UpdateOld: []*endpoint.Endpoint{
					NewEndpoint("update.example.com", "1.2.3.4", 3600, nil),
				},
				UpdateNew: []*endpoint.Endpoint{
...
func TestMikrotikProvider_GetDomainFilter(t *testing.T) {
	...
	if result != domainFilter {
		t.Errorf("Expected domain filter to be %v, got %v", domainFilter, result)
	}

	// Test that the returned filter actually works
	if !result.Match("example.com") {
		t.Error("Expected example.com to match domain filter")
	}
	if !result.Match("sub.example.com") {
		t.Error("Expected sub.example.com to match domain filter")
	}
